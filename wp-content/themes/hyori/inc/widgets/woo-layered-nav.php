...
class Hyori_Widget_Woo_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display a list of attributes to filter products in your store.', 'hyori' );
		$this->widget_id          = 'goal_woocommerce_layered_nav';
		$this->widget_name        = esc_html__( 'Goal Filter Products by Attribute', 'hyori' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			...
			'attribute' => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => esc_html__( 'Attribute', 'hyori' ),
				'options' => $attribute_array,
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => esc_html__( 'Query type', 'hyori' ),
				'options' => array(
					'and' => esc_html__( 'AND', 'hyori' ),
					'or'  => esc_html__( 'OR', 'hyori' ),
				),
			),
		);
	}
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				$term_html = '<a href="' . $link . '">' . self::display_swatch($attr->attribute_type, $value) . esc_html( $term->name ) . $term_count_html . '</a>';
			} else {
				$link      = false;
...
if ( function_exists('goal_framework_reg_widget') ) {
	goal_framework_reg_widget('Hyori_Widget_Woo_Layered_Nav');
}