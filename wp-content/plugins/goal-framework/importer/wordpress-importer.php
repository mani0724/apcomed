...
class WP_Import extends WP_Importer {
	...
	public function saveCaches(){
		foreach( $this->caches as $cache ){
			if( isset($this->$cache) ){
				file_put_contents( GOAL_IMPORT_SAMPLES_DIR.$cache.'.txt', json_encode( $this->$cache ) );
			}
		}
	}
	...
	public function deleteCaches(){
		foreach( $this->caches as $cache ){
			if(  is_file( GOAL_IMPORT_SAMPLES_DIR.$cache.'.txt' ) ){
				@unlink(  GOAL_IMPORT_SAMPLES_DIR.$cache.'.txt' );
			}
		}
	}
	public function getCaches(){
		foreach( $this->caches as $cache ){
			if( file_exists( GOAL_IMPORT_SAMPLES_DIR.$cache.'.txt' ) ){
				$this->$cache = json_decode( file_get_contents( GOAL_IMPORT_SAMPLES_DIR.$cache.'.txt' ), true );
			}
		}
	}
...
<?php
	...
	function process_posts() {
		...
		$posts = array_chunk( $this->posts, 10 );
			...
			if ( ! empty( $post['postmeta'] ) ) {
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {
						...
						if ( $key == '_elementor_data' && $value ) {
							$json_value = json_decode( stripslashes_deep($value) );
							$json_value = wp_slash( wp_json_encode( $json_value ) );
							
							update_metadata( 'post', $post_id, $key, $json_value );
						} else {
