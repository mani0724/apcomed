...
if( ! class_exists( 'CMB2_Field_Ajax_Search' ) ) {
	...
	class CMB2_Field_Ajax_Search {
		...
		public function __construct() {
			...
			// Ajax endpoints.
			add_action( 'wre_ajax_cmb_ajax_search_get_results', array( $this, 'get_results' ) );
		...
		public function get_results() {
			...
			if ( ! wp_verify_nonce( $nonce, 'cmb_ajax_search_get_results' ) ) {
				...
				die( json_encode( array(
                    'error' => __( 'Error : Unauthorized action', 'wp-salespopup' )
                ) ) );
                ...
                die( json_encode( array(
                    'error' => __( 'Error : Wrong request parameters', 'wp-salespopup' )
                ) ) );
            } else {
		...
		public function object_text_html( $field_id, $object_id, $object_type ) {
			...
			if( $object_type == 'post' ) {
				...
				if ( WP_RealEstate_User::is_agent($object_id) ) {
					$agent_id = WP_RealEstate_User::get_agent_by_user_id($object_id);
					$avatar = get_the_post_thumbnail( $agent_id, 'thumbnail' );
					$title = get_the_title($agent_id).' ('.get_the_author_meta('display_name', $object_id).')';
				} elseif ( WP_RealEstate_User::is_agency($object_id) ) {
					$agency_id = WP_RealEstate_User::get_agency_by_user_id($object_id);
					$avatar = get_the_post_thumbnail( $agency_id, 'thumbnail' );
					$title = get_the_title($agency_id).' ('.get_the_author_meta('display_name', $object_id).')';
				} else {
